...
- **MCP server:** Instatic exposes its CMS tools to external MCP clients (Claude Code, Codex, remote agents) at `/_instatic/mcp`, authenticated by per-connector bearer tokens. Thin adapter over the existing tool engine: headless tools (content reads, `read_page_tree`/`mutate_page_tree` via `server/ai/content/treeService.ts`) run in-process; the full set of browser-execution editor tools is relayed to the connector owner's **open editor** via the live editor bridge (`server/ai/mcp/editorBridge.ts` + `useEditorMcpBridge` in `SitePage`), reusing the chat bridge machinery. Feature doc: [`docs/features/mcp-connectors.md`](docs/features/mcp-connectors.md).
