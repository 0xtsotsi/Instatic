...
- **MCP server:** Instatic exposes its CMS tools to external MCP clients (Claude Code, Codex, remote agents) at `/_instatic/mcp`, authenticated by per-connector bearer tokens. Thin adapter over the existing tool engine + the headless page-tree service (`server/ai/content/treeService.ts`, shared with the plugin tree RPC). Feature doc: [`docs/features/mcp-connectors.md`](docs/features/mcp-connectors.md).
