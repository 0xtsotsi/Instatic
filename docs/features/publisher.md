...
VC ref subtlety (Rule 4): when a VC definition tree is dynamic, the _outer_ `base.visual-component-ref` node id in the page tree goes into `dynamicPageNodeIds` — not the inner VC node ids. The hole boundary is the ref, not any inner node.
...
Hole shells are stamped with the _next_ publish version (`getPublishVersion() +
