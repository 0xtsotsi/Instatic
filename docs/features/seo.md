...
| `robots.ts` | `generateRobotsTxt` — shared by the server endpoint and the admin live preview (byte-identical). Composes one group per user-agent from: the `*` group (with default system-path disallows), the AI-crawler toggles, and custom `rules`; appends raw `extraDirectives`; `SYSTEM_DISALLOW_PATHS` is the opt-out default list. `blockAll` serves a bare `Disallow: /` for preview hosts. |
| `robotsAnalysis.ts` | `lintRobotsTxt` (flags unknown directives, rules before any `User-agent`, malformed values) + `matchRobots` (is a path crawlable for a UA — Google longest-match, `Allow` tie-break, `*`/`$` support). Powers the Robots tab's lint panel and URL tester. |
...
- `robots.txt` (`text/plain`): default allow with system-path disallows
  (`/admin`, `/_instatic/`), optional per-bot `Disallow` blocks from the two
  AI-crawler toggles (training bots: GPTBot, Google-Extended, CCBot,
  Applebot-Extended, meta-externalagent; answer bots: OAI-SearchBot,
  PerplexityBot, ChatGPT-User, Claude-SearchBot), custom per-UA rules, raw
  extra directives, and a `Sitemap:` line.
...
- **Robots.txt tab** — three settings cards (crawling toggles: indexing,
  system-path block, AI training/answer crawlers · custom per-UA Allow/
  Disallow rule editor · an Advanced raw-directives field) beside a sticky
  live preview generated by the same `generateRobotsTxt` the endpoint
  serves, a lint panel (`lintRobotsTxt`), and a "test a URL" checker
  (`matchRobots`).
