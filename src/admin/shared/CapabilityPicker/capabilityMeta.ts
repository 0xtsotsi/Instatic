...
export const CAPABILITY_META: Record<CoreCapability, CapabilityMeta> = {
  ...
  'site.content.edit': {
    ...
    description:
      'Change text, images, and links on existing nodes — no structure or style changes.',
  },
  'site.style.edit': {
  ...
  'media.write': {
    ...
    description:
      'Upload assets, edit metadata (alt text, caption, tags), manage folders, restore from trash.',
  },
  'media.replace': {
    label: 'Replace media bytes',
    description:
      'Overwrite the bytes of an existing asset (variants regenerate). Uniquely powerful — silently swaps the file every reference points at.',
  },
  'media.delete': {
    label: 'Delete media',
    description:
      'Soft-delete assets to trash; hard-purge (also requires step-up) removes the bytes from disk.',
  },
  // ---------------------------------------------------------------------
  ...
  'storage.elect': {
    ...
    description:
      'Elect the media storage adapter per asset role (originals / variants / avatars / fonts) and the variant delegate.',
  },
  'storage.migrate': {
    label: 'Migrate storage bytes',
    description:
      'Run the migration SSE that moves bytes between storage adapters after an election change.',
  },
  // ---------------------------------------------------------------------
  ...
  'plugins.install': {
    ...
    description:
      'Install, upgrade, and uninstall plugins. Runs third-party code on the host — RCE-class. Step-up gated.',
  },
  'plugins.lifecycle': {
    label: 'Plugin lifecycle control',
    description:
      'Enable, disable, restart plugins. Pause/resume/run-now their schedules. Step-up gated.',
  },
  'users.manage': {
    label: 'Manage users',
  ...
  'data.custom.tables.read': {
    ...
    description:
      'Open the Data workspace; browse user-created (custom) tables and their field schemas. Does not reveal the internal system tables.',
  },
  'data.custom.tables.manage': {
    label: 'Manage custom tables',
    description:
      'Create, rename, and delete custom tables; add/edit/remove their fields and route bases.',
  },
  'data.system.tables.read': {
    label: 'Browse system tables',
    description:
      'See and open the four built-in system tables (Posts, Pages, Components, Layouts) in the Data workspace.',
  },
  'data.system.tables.manage': {
    label: 'Manage system-table custom fields',
    description:
      'On system tables: add/edit/remove custom fields and choose the primary field. Built-in fields and the table identity (name, slug, route) stay locked.',
  },
  'data.rows.move': {
    label: 'Move rows between tables',
    description:
      'Move a row from one table to another (changes public URL because route base differs per table).',
  },
  'data.export': {
    label: 'Export data bundles',
    description:
      'Download a JSON bundle of tables + rows (plus optional media bytes). Includes the import preview dry-run.',
  },
  'data.import': {
    label: 'Import data bundles',
    description:
      'Upload a JSON bundle to merge or replace local data. `replace` mode additionally requires Manage content and step-up.',
  },
  // ---------------------------------------------------------------------
  ...
  'ai.chat': {
    label: 'Use AI chat',
    description:
      'Open AI conversations and use read-only tools (snapshot, search). Cannot mutate state without `Allow AI write tools`.',
  },
  'ai.tools.write': {
    label: 'Allow AI write tools',
    description:
      'Lets AI conversations mutate the editor store (insert nodes, edit props, etc.) via the canvas bridge. Without this, the model has no write tools registered.',
  },
  'ai.providers.manage': {
