...
export const MODULE_WIRES: Readonly<Record<string, WireNode>> = {
  ...
  'base.outlet': col(
    [lines(1, { big: true, width: 60 }), lines(3), image({ height: 30 }), lines(2)],
    { gap: 6 },
  ),
  'base.form': col([field(), field(), button({ width: 40 })], { gap: 6 }),
  'base.form-message': box([row([icon(), lines(1, { flex: 1 })], { gap: 6, align: 'center' })], {
    pad: 6,
    message: true,
  }),
  ...
  'base.slot-outlet': box([lines(1, { width: 52, center: true })], {
    dashed: true,
    height: 44,
    center: true,
  }),
