...
export function TokenAwareInput({
  ...
  const suggestions = !q
    ? []
    : tokens
        .filter((t) => t.step.toLowerCase().startsWith(q) || t.step.toLowerCase().includes(q))
        .slice(0, 8)
  ...
  return (
    <div className={cn(overlay ? styles.wrapperOverlay : styles.wrapper, className)} style={style}>
      {tooltipOnOverflow ? (
        <Tooltip content={draft} side="top" disabled={!isOverflowing || isEditing || !draft}>
          {inputEl}
        </Tooltip>
      ) : (
        inputEl
