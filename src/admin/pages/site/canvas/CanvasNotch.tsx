...
} & ({ moduleId: string; icon?: never } | { icon: IconComponent; moduleId?: never })
    ...
    >
      ...
      <div className={styles.roller}>
        <div className={styles.notch}>
          ...
          {actions ? actions.map((action) => renderActionButton(action)) : <FavoriteNotchActions />}
        ...
        {floatingControl && <div className={styles.floatingControl}>{floatingControl}</div>}
      </div>
    </div>
  )
}
...
function FavoriteNotchActions() {
  ...
  function insertComponent(componentId: string) {
    ...
    const location = resolveInsertLocation(canvasPage, selectedNodeId ?? canvasPage.rootNodeId)
    if (!location) return
  ...
  const menuIndex = menu ? favoriteItems.findIndex((fav) => fav.key === menu.item.key) : -1
  ...
  return (
    ...
    >
      {ActionIcon ? (
        ...
        <ModuleIcon moduleId={action.moduleId} size={14} aria-hidden="true" />
      )}
    </Button>
  )
}
