...
interface VisualComponentsSlice {
  ...
  updateParamMeta(
    vcId: string,
    paramId: string,
    patch: { required?: boolean; description?: string; enumOptions?: string[] },
  ): void
...
export const createVisualComponentsSlice: EditorStoreSliceCreator<VisualComponentsSlice> = (
  set,
  get,
) => {
  ...
  const { mutateSite, mutateSiteState, mutateSiteWithExplorerReconcile } = buildSiteHelpers(
    set,
    get,
  )
  ...
  return {
    ...
    updateParamDefaultValue(vcId, paramId, value) {
      mutateSite(
        ...
        { coalesceKey: `vcparam:${vcId}:${paramId}` },
      ) // coalesce per-keystroke edits
    },
