...
describe('base form primitive modules', () => {
  ...
  it('falls back to fieldId for the name attribute when name is blank', () => {
    expect(
      InputModule.render({ ...InputModule.defaults, fieldId: 'email', name: '' }).html,
    ).toContain('name="email"')
    expect(
      TextareaModule.render({ ...TextareaModule.defaults, fieldId: 'bio', name: '' }).html,
    ).toContain('name="bio"')
    expect(
      SelectModule.render({ ...SelectModule.defaults, fieldId: 'country', name: '' }, []).html,
    ).toContain('name="country"')
    expect(
      CheckboxModule.render({ ...CheckboxModule.defaults, fieldId: 'optin', name: '' }, []).html,
  ...
  it('renders labels and text-like controls as semantic HTML', () => {
    expect(LabelModule.render({ text: 'Email', targetMode: 'auto', targetId: '' }, []).html).toBe(
  ...
  it('renders textarea, select, option, and option-group primitives', () => {
    expect(
      ...
      '<textarea data-instatic-form-control="textarea" data-instatic-field-id="message" name="message" id="message-input" placeholder="Message" rows="4" maxlength="500">Hello</textarea>',
    )

    expect(
      OptionModule.render({ value: 'pro', label: 'Pro', selected: true, disabled: false }, []).html,
    ).toBe('<option value="pro" selected>Pro</option>')

    expect(
      OptionGroupModule.render({ label: 'Plans', disabled: false }, ['<option>Pro</option>']).html,
    ...
    expect(
      ...
      '<select data-instatic-form-control="select" data-instatic-field-id="plan" name="plan" id="plan-select" required><option value="pro">Pro</option></select>',
    )
  })

  it('renders choice controls, submit, and form messages', () => {
    expect(
      CheckboxModule.render(
        {
          fieldId: 'agree',
          name: 'agree',
          id: 'agree-input',
      ...
      '<input type="checkbox" data-instatic-form-control="checkbox" data-instatic-field-id="agree" name="agree" id="agree-input" value="yes" checked required>',
    )
    ...
    expect(
      ...
      '<input type="radio" data-instatic-form-control="radio" data-instatic-field-id="plan" name="plan" id="plan-pro" value="pro">',
    )

    expect(SubmitModule.render({ label: 'Subscribe', disabled: false, formId: '' }, []).html).toBe(
      '<button type="submit">Subscribe</button>',
    )

    expect(
      FormMessageModule.render(
        {
          formId: 'newsletter',
          kind: 'success',
          text: 'Thanks',
      ...
      '<div data-instatic-form-message="success" data-instatic-form-id="newsletter" role="status">Thanks</div>',
    )
  })
  ...
  it('normalizes configured form ids while preserving safe custom-action URLs', () => {
    ...
    expect(
      SubmitModule.render({ ...SubmitModule.defaults, formId: 'Contact Form!' }, []).html,
    ).toBe('<button type="submit" form="Contact-Form">Submit</button>')
    expect(
      FormMessageModule.render({ ...FormMessageModule.defaults, formId: 'Contact Form!' }, []).html,
