...
describe('server router', () => {
  it('serves health checks', async () => {
    const res = await handleServerRequest(new Request('http://localhost/health'), {
      db: makeFakeDb(),
    })
  ...
  it('returns 404 for unknown routes once setup is complete', async () => {
    const res = await handleServerRequest(new Request('http://localhost/nope'), {
...
describe('server router — Layer A disk artefact fast-path', () => {
  ...
  it('serves a baked disk artefact without a DB snapshot lookup', async () => {
    ...
    const originalHandle = db as unknown as (
      strings: TemplateStringsArray,
      ...values: unknown[]
    ) => Promise<DbResult>
    ...
    const res = await handleServerRequest(new Request('http://localhost/about'), {
      db: trackingDb,
      uploadsDir,
    })
  ...
  it('falls through to the resolver when no artefact exists for the URL', async () => {
    ...
    const res = await handleServerRequest(new Request('http://localhost/contact'), {
      db: makeFakeDb({ site: 1, owners: 1 }),
      uploadsDir,
    })
