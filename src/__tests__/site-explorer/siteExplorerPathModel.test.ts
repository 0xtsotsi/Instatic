...
describe('buildStructuralExplorerTreeSection', () => {
  it('builds recursive page folders from slash slugs', () => {
    ...
    const model = buildStructuralExplorerTreeSection('pages', explorer.pages, [
      {
        id: 'home',
        label: 'Home',
        path: 'index',
    ...
    expect(
      docs.children.map((child) => (child.kind === 'item' ? child.item.id : child.folder.path)),
    ).toEqual(['setup'])
  })

  it('builds script folders from file paths', () => {
    const explorer = createDefaultSiteExplorerOrganization()
    const model = buildStructuralExplorerTreeSection('scripts', explorer.scripts, [
      {
        id: 'jquery',
        label: 'jquery.min.js',
    ...
    if (!documentation || documentation.kind !== 'folder')
      throw new Error('Expected documentation folder')
    expect(documentation.children[0]).toMatchObject({
      kind: 'folder',
      folder: { path: 'documentation/assets' },
    })
  })
})
