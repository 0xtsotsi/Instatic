...
describe('chatCompletions shared adapter', () => {
  ...
  it('normalizeOpenAiBaseUrl strips trailing /v1 so it is not doubled when building the endpoint', () => {
    ...
    expect(normalizeOpenAiBaseUrl('https://api.groq.com/openai/v1')).toBe(
    ...
    expect(normalizeOpenAiBaseUrl('https://api.groq.com/openai')).toBe(
      'https://api.groq.com/openai',
    )
  ...
  it('still emits text when delta.tool_calls is null', () => {
    ...
    const events = t.translate(
      frame({
        choices: [
          {
            delta: { content: 'Hi', reasoning_content: null, tool_calls: null },
            finish_reason: 'stop',
          },
        ],
        usage: null,
  ...
  it('captures the final content of a reasoning model (content empty during reasoning, filled at the end)', () => {
    ...
    t.translate(
      frame({
        choices: [{ delta: { content: '', reasoning_content: 'thinking…', tool_calls: null } }],
        usage: null,
      }),
    )
    t.translate(
      frame({ choices: [{ delta: { content: null, reasoning_content: ' more' } }], usage: null }),
    )
    // Final answer arrives in content.
    const last = t.translate(
      frame({
        choices: [{ delta: { content: 'Hello there!', tool_calls: null }, finish_reason: 'stop' }],
        usage: null,
      }),
    )
    ...
    const result = t.finish()
    expect(result.stop).toBe(true)
    expect(result.assistantMessage?.[0]).toMatchObject({
      role: 'assistant',
      content: 'Hello there!',
    })
  })
  ...
  it('translator emits one toolCall event per accumulated call at finish_reason', () => {
    ...
    const ev = t.translate(
      frame({
        choices: [
          {
            delta: { tool_calls: [{ index: 0, function: { arguments: 'ml":"<p/>"}' } }] },
